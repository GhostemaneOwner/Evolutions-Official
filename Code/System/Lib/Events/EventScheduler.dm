...
EventScheduler
	proc
		...
		schedule(var/Event/E, var/ticks as num, var/priority = 0)
			...
			if (A) A += T
			else src.__scheduled_events[ticks] = list(T)
		...
		start()
			if (!src.__running)
				src.__running = 1
				if(src.null) cancel() //Testing a more efficient fix to remove the null errors.
			//	if(!src)  //Added in part 3 update 2 of V.033 to attempt to stop null errors. Temporarily removed for testing in
			//		return  //Added in part 3 update 2 of V.033 to attempt to stop null errors. Temporarily removed for testing in
			//	else  //Added in part 3 update 2 of V.033 to attempt to stop null errors.  Temporarily removed for testing in
				spawn() src.__loop()
