...
/* Recode this later
	for(var/mob/M in players)
		M.TryUnlockForm("Omega Saiyan God")
		M.TryUnlockForm("Omega Saiyan Blue")
		M.TryUnlockForm("Omega Saiyan Blue Evolved") //Super Saiyan
...
/mob/proc/GainTraitPoints()
	var/expectedPoints = Math.Floor(2) + bonusTraitPoints
...
trait
	...
	Kill_You
		...
		CanGainRank(mob/M)
			return ..() && ((M.Race in list("Saiyan", "Half-Saiyan", "Demon", "Majin")) || M.HasTrait("Saiyan Genome"))

		ListReqs()
			var/html = "Race must be one of the following: <ul>"
			html += "<li>Saiyan</li>"
			html += "<li>Half-Saiyan</li>"
	...
	Monkey_Control
		...
		CanGainRank(mob/M)
			return ..() && M.Race in list("Saiyan", "Half Saiyan")

		SpecialGainEvent(mob/M)
			if(!M || !M.client) return

		ListReqs()
			var/html = "Must be a Saiyan or Half Saiyan."
			return html
